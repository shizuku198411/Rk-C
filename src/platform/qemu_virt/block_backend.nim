## Provides QEMU virtio-blk block device access.
import ../../arch/riscv64/arch
import ../../kernel/dev/console
import ../../kernel/lib/virtio
import ../../lib/types
...
## Reads the virtio-blk capacity from device config space.
...
## Recovers the virtio-blk queue after an I/O timeout.
...
## Submits one virtio-blk request and waits for completion.
...
## Runs a virtio-blk request with timeout recovery.
...
## Initializes the QEMU virtio-blk backend.
proc init*(outCapacityBlocks: var U64): int =
  ...
  initialized = true
  outCapacityBlocks = capacityBlocks
  0
...
## Reads one physical block from the QEMU virtio-blk backend.
...
## Writes one physical block to the QEMU virtio-blk backend.
