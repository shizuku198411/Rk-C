...
def normal_tests() -> list[TestCase]:
    ...
    tests.extend(
        [
            ...
            TestCase("ls -l chown user", "ls -l /tmp", ["-rw-r--r--", "user:user", "chown_user_file"]),
...
def user_tests() -> list[TestCase]:
    return [
        ...
        TestCase("user history saved under home", "cat /home/.history", ["pwd", "exit"]),
        TestCase("user history typo path absent", "cat /home.history", ["cat: failed"]),
        TestCase("root history saved under root", "cat /.history", ["su user"]),
        ...
        TestCase("user chowned file mode visible", "ls -l /tmp", ["-rw-------", "user:user", "chown_user_file"]),
        ...
        TestCase("user-owned tmp file visible", "ls -l /tmp", ["-rw-r--r--", "user:user", "user_sticky"]),
        ...
        TestCase("user-owned home file", "ls -l /home", ["-rw-r--r--", "user:user", "note.txt"]),
        TestCase("user chmod own file", "chmod 600 note.txt", []),
        TestCase("user chmod own file visible", "ls -l /home", ["-rw-------", "user:user", "note.txt"]),
