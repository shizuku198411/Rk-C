...
def runtime_tests() -> list[TestCase]:
    ...
    return [
        ...
        TestCase(
            "ls /proc",
            "ls /proc",
            ["uptime", "cpuinfo", "mounts", "kmsg", "3/", "4/", "5/", "6/", "7/", "8/", "9/"],
            not_contains=["./", "../"],
            regex=[r"\d+/"],
        ),
        TestCase(
        ...
        TestCase("cat /proc/meminfo", "cat /proc/meminfo", ["total:", "used :", "free :", "next_free_hint:"]),
        TestCase("cat /proc/mounts", "cat /proc/mounts", ["name", "fs", "mount", "flags", "rootfs", "tmpfs", "appfs", "/bin", "ro", "rw"]),
        ...
        TestCase("shell ls proc cwd", "ls", ["uptime", "cpuinfo", "mounts", "kmsg"], not_contains=["./", "../"], regex=[r"\d+/"]),
        TestCase("shell cd proc pid", "cd 1", []),
        TestCase("shell pwd after cd proc pid", "pwd", ["/proc/1"]),
        TestCase("shell ls proc pid cwd", "ls", ["status", "rkx_map", "wait"], not_contains=["./", "../"]),
        ...
        TestCase("ls /proc/1", "ls /proc/1", ["status", "rkx_map", "wait"], not_contains=["./", "../"]),
        TestCase("cat /proc/1/status", "cat /proc/1/status", ["pid: 1", "uid: 0", "gid: 0", "cpu:", "mem:", "wait:", "exe: init"]),
        TestCase("cat /proc/1/wait", "cat /proc/1/wait", ["pid: 1", "state:", "wait:", "wait_value:"]),
